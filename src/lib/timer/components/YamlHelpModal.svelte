    ...
    <section class="modal" role="document">
      ...
      <div class="modal__content">
        ...
        <ul>
          ...
          <li><code>restAfterSeconds</code> (number, optional, default 0): Rest between repeats of the same round.</li>
          <li><code>transitionAfterSeconds</code> (number, optional): Transition/rest after the round finishes, before the next round. When omitted, the app uses <code>restAfterSeconds</code> for backwards compatibility.</li>
    ...
    sets:
      ...
      - label: Plank
        ...
        <p>
          ...
          between sets, <code>restSeconds</code> for breaks between repetitions of the same set,
          <code>restAfterSeconds</code> for breaks between round repeats, and <code>transitionAfterSeconds</code> for breaks between rounds.
        </p>
      </div>
    </section>
  </div>
